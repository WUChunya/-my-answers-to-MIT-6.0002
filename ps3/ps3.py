...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for i in range(self.width):
            for j in range(self.height):
                self.Room_graph[i, j] = dirt_amount
    ...
    def is_tile_cleaned(self, m, n):
        ...
        """
        room_graph = self.get_Room_graph()
        return room_graph[m, n] == 0

    def get_num_cleaned_tiles(self):
        """
        Returns: an integer; the total number of clean tiles in the room
        """
        return list(self.get_Room_graph().values()).count(0)
    ...
    def is_position_in_room(self, pos):
        ...
        width = self.width
        height = self.height        
        return width > pos.get_x() >= 0  and height > pos.get_y() >= 0 
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        ram_x = random.randint(0,self.width) + random.random()
        ram_y = random.randint(0,self.height) + random.random()
        return Position(ram_x, ram_y)
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        """
        return (self.is_position_valid(pos) and not self.is_position_furnished(pos) )
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self): 
        ...
        """
        #loop until all tiles were cleaned
        room = self.room
        
        #caculate the new position
        origin_position = self.get_robot_position()
        origin_direction = self.get_robot_direction()            
        new_position = origin_position.get_new_position(origin_direction, self.speed)
        
        #if the new location available,move to the location and clean that tile
        if room.is_position_valid(new_position):
            self.set_robot_position(new_position)
            room.clean_tile_at_position(self.position, self.capacity)
        #if the new location unavailable, change to another direction
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        #if the new location available and robot works normally,move to the location and clean that tile
        if room.is_position_valid(new_position) and not self.gets_faulty():
            self.set_robot_position(new_position)
            room.clean_tile_at_position(self.position, self.capacity)
            
        #if the new location unavailable, or the robot broke, change to another direction without cleaning        
        else:
            new_direction = random.randint(0,360)
            self.set_robot_direction(new_direction) 
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                FaultyRobot)
    """  
    #test num_robots in num_trials
    count_collection = []
    is_furnished = False
    delay = 0.2
    anim = ps3_visualize.RobotVisualization(num_robots, width, height, is_furnished, delay)
    for m in range(num_trials):
        # create a new room and basic conditions
        room = EmptyRoom(width, height, dirt_amount)
        test_robot = robot_type(room, speed, capacity)
        min_clean_tiles = math.ceil(min_coverage*room.get_num_tiles())
        count = 0
        
        #set up robots randomly
        robots = []
        for n in range(num_robots):
            test_robot.set_robot_position(room.get_random_position()) 
            robots.append(test_robot)
            
        #test the robot
        while room.get_num_cleaned_tiles() < min_clean_tiles:   
            for robot in robots:    
    ...
    #calculate the average time_steps(==count)   
    anim.done()
    return mean(count_collection)
    
 

print ('avg time steps: ' + str(run_simulation(3, 1.0, 1, 8, 8, 3, 1.0, 50, StandardRobot)))
...
#       of a 20x20 room?
# 1. Standard robot and Faulty robot behaves the same when number of robots increase:the cost time descrease
# 2. Adding the amount of robot, the difference between two kinds of robots decrease
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
# 1. space ratio of 1.33(20*15) is the lowest point of ratio and time/step graph
# 2. in whatever place, faulty robots cost much more time/steps than standard ones
